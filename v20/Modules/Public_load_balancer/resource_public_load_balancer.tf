...
/* # Create health check availability probe for https
resource "azurerm_lb_probe" "probe" {
  resource_group_name = var.resource_group_name
  loadbalancer_id     = azurerm_lb.load_balancer.id
  name                = var.lb_services
  port                = var.lb_svc
# request_path        = "/"
}
...
resource "azurerm_lb_rule" "lb_rule_https" {
  resource_group_name            = var.resource_group_name
  loadbalancer_id                = azurerm_lb.load_balancer.id
  name                           = azurerm_lb_probe.https_probe.name
  protocol                       = "Tcp"
  frontend_port                  = azurerm_lb_probe.https_probe.port
  backend_port                   = azurerm_lb_probe.https_probe.port
  frontend_ip_configuration_name = azurerm_public_ip.public_ip.name
  probe_id                       = azurerm_lb_probe.https_probe.id
  backend_address_pool_id        = azurerm_lb_backend_address_pool.backend_address_pool.id
}

/* # Create health check availability probe for http
...
resource "azurerm_lb_rule" "lb_rule_http" {
  resource_group_name            = var.resource_group_name
  loadbalancer_id                = azurerm_lb.load_balancer.id
  name                           = azurerm_lb_probe.http_probe.name
  protocol                       = "Tcp"
  frontend_port                  = azurerm_lb_probe.http_probe.port
  backend_port                   = azurerm_lb_probe.http_probe.port
  frontend_ip_configuration_name = azurerm_public_ip.public_ip.name
